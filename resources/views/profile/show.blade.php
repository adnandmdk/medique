        ...
        <div class="profile-hero">
            ...
            <div style="flex:1;min-width:0;">
                ...
                <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap;">
                    <span style="background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.25);padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;">
                        {{ ucfirst($user->role) }}
                    </span>
                    @if($user->hospital)
                        <span style="background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.25);padding:3px 10px;border-radius:20px;font-size:11px;">
                            {{ $user->hospital->name }}
                        </span>
                    @endif
                ...
                @if($user->isPatient())
                    ...
                    <div class="form-group">
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                            <label class="check-label" id="lm" style="border-color:{{ old('gender',$user->gender)==='male'?'var(--brand)':'var(--border)' }};background:{{ old('gender',$user->gender)==='male'?'var(--brand-light)':'var(--surface)' }};">
                            ...
                            <label class="check-label" id="lf" style="border-color:{{ old('gender',$user->gender)==='female'?'var(--brand)':'var(--border)' }};background:{{ old('gender',$user->gender)==='female'?'var(--brand-light)':'var(--surface)' }};">
        ...
        @if($queues->count() > 0)
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('patient.queues.index') }}" class="btn btn-secondary btn-sm">Lihat Semua</a>
                </div>
                ...
                @foreach($queues as $q)
                    <div style="padding:11px 16px;border-bottom:1px solid #F8FAFC;display:flex;align-items:center;gap:10px;">
                        <div style="font-size:14px;font-weight:800;color:var(--brand);min-width:72px;">{{ $q->queue_number }}</div>
                        <div style="flex:1;min-width:0;">
                        ...
                        <span class="badge {{ $sm[$q->status] ?? 'badge-inactive' }}">{{ $q->status_label }}</span>
                    </div>
                @endforeach
            </div>
        @endif
