<aside class="sidebar" id="appSidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <div class="nav-section">
            ...
            <a href="{{ route('admin.queues.index') }}" class="nav-item {{ request()->routeIs('admin.queues.*') ? 'active' : '' }}">
        ...
        <div class="nav-section">
            ...
            <a href="{{ route('doctor.queues.index') }}" class="nav-item {{ request()->routeIs('doctor.queues.*') ? 'active' : '' }}">
                ...
                Antrian Hari Ini
                @php $myDoc=$_authDoctor??null; $mq=$myDoc?\App\Models\Queue::whereHas('schedule',fn($q)=>$q->where('doctor_id',$myDoc->id))->where('booking_date',today())->whereIn('status',['waiting','called'])->count():0; @endphp
                @if($mq>0)<span class="nav-badge green">{{ $mq }}</span>@endif
            </a>
        ...
        <div class="nav-section">
            ...
            <a href="{{ route('patient.queues.index') }}" class="nav-item {{ request()->routeIs('patient.queues.index') ? 'active' : '' }}">
                <svg viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/></svg>
                Antrian Saya
            </a>
            <a href="{{ route('patient.queues.create') }}" class="nav-item {{ request()->routeIs('patient.queues.create') ? 'active' : '' }}">
