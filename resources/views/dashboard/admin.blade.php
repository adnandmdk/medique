<x-app-layout title="Dashboard Admin">
    <x-slot name="header">
        ...
        <div class="topbar-subtitle">Ringkasan sistem</div>
    </x-slot>
    <x-slot name="actions">
        <a href="{{ route('admin.queues.index') }}" class="btn btn-secondary btn-sm">Monitor Antrian</a>
    </x-slot>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-sub">
                {{ $stats['done_queues'] ?? 0 }} selesai ·
                {{ $stats['waiting_queues'] ?? 0 }} menunggu
            </div>
        </div>
    </div>
    ...
    <div class="quick-grid">
        <a href="{{ route('admin.clinics.index') }}" class="quick-card">
            <div class="quick-icon" style="background:var(--brand-light);">
                <svg viewBox="0 0 24 24" stroke="var(--brand)"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg>
            </div>
            <div class="quick-label">Poliklinik</div>
        </a>
        <a href="{{ route('admin.doctors.index') }}" class="quick-card">
            <div class="quick-icon" style="background:#E6F1FB;">
                <svg viewBox="0 0 24 24" stroke="#185FA5"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            </div>
            <div class="quick-label">Dokter</div>
        </a>
        <a href="{{ route('admin.schedules.index') }}" class="quick-card">
            <div class="quick-icon" style="background:#EEEDFE;">
                <svg viewBox="0 0 24 24" stroke="#534AB7"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            </div>
        ...
        <a href="{{ route('admin.queues.index') }}" class="quick-card">
            <div class="quick-icon" style="background:var(--accent-light);">
                <svg viewBox="0 0 24 24" stroke="var(--accent)"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/></svg>
            </div>
    ...
    <div class="card">
        <div class="card-header">
            <div>
                ...
                <div class="card-subtitle">5 antrian terakhir masuk</div>
            </div>
        ...
        <table>
            <thead>
                <tr>
                    <th>No</th><th>Token</th><th>Pasien</th><th>Dokter</th><th>Poliklinik</th><th>Status</th>
                </tr>
            </thead>
            <tbody>
                @php
                ...
                @forelse($stats['recent_queues'] ?? [] as $queue)
                    @php
                        [$bg, $fg] = $avatarColors[$loop->index % count($avatarColors)];
                        $initials = strtoupper(substr(optional($queue->patient)->name ?? 'NA', 0, 2));
                    @endphp
                    <tr>
                        ...
                        <td>
                            <span class="badge {{ $statusMap[$queue->status] ?? 'badge-inactive' }}">
                                {{ $queue->status_label }}
                            </span>
                        </td>
                    </tr>
